...
class Updater:
    def __init__(self, check_interval=300, check_on_startup=True, http_proxy="", https_proxy=""):
        ...
        self.service_name="ghostnode"
        self.current_version=VERSION
        arch=platform.machine()
        arch_suffix="-arm64" if arch in ("aarch64","arm64") else ""
        self.update_url=f"https://github.com/{GITHUB_REPO}/releases/latest/download/ghostnode{arch_suffix}"
    ...
    async def download_update(self, new_version):
        try:
            checksum_url=f"{self.update_url}.sha256"
            logger.info(f"Downloading update from {self.update_url}")
            tmpdir=f"/tmp/ghostnode-update-{os.getpid()}"
            os.makedirs(tmpdir, exist_ok=True)
            binary_path=os.path.join(tmpdir, "ghostnode")
